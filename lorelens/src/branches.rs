    ...
    pub(super) fn branch_menu(&self, enabled: bool, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        Button::new("branch-menu")
            ...
            .dropdown_menu(move |mut menu, window, cx| {
                ...
                for (label, command, title) in
                    [("Update / Sync", "sync", "Sync"), ("Push…", "push", "Push")]
                {
                    let view = view.clone();
                    let root = root.clone();
                ...
                menu = menu.item(PopupMenuItem::new(t("Switch local branch…")).on_click(
                    move |_, window, cx| {
                        let _ = switch_view
                            .update(cx, |this, cx| this.switch_branch_dialog(window, cx));
                    },
                ));
                for (label, is_remote) in
                    [("New local branch…", false), ("New remote branch…", true)]
                {
                    let view = view.clone();
                    menu =
                        menu.item(PopupMenuItem::new(t(label)).on_click(move |_, window, cx| {
                            let _ = view.update(cx, |this, cx| {
                                this.new_branch_dialog(is_remote, window, cx)
                            });
                        }));
                }
                ...
                for branch in &local {
                    ...
                    menu = menu.submenu(label, window, cx, move |submenu, _, _| {
                        ...
                        submenu
                            .item(
                                PopupMenuItem::new(t("Switch to branch"))
                                    .disabled(is_current)
                                    .on_click(move |_, _, cx| {
                                        let _ = switch_view.update(cx, |this, cx| {
                                            if this.root == switch_root
                                                && this.local_branches.contains(&source)
                                            {
                                                this.command(
                                                    vec![
                                                        "branch".into(),
