...
pub fn parse_status(output: &str) -> Result<Status, String> {
    ...
    for line in output.lines().filter(|s| !s.trim().is_empty()) {
        let event: Value =
            serde_json::from_str(line).map_err(|e| format!("Invalid Lore JSON: {e}"))?;
        let data = &event["data"];
    ...
    if !found {
        return Err(
            "Lore did not report a repository. Open a Lore repository, or check the CLI version."
                .into(),
        );
    }
...
pub fn pending_push(
    ...
    let data = &events
        ...
        .ok_or("Missing repository status")?["data"];
    let flag = |name: &str| {
        data[name]
            .as_bool()
            .unwrap_or_else(|| data[name].as_u64() == Some(1))
    };
    ...
    let read_history = |revision: &str| -> Result<Vec<(String, String)>, String> {
        let output = run_as(
            cli,
            root,
            &[
    ...
    let local = read_history(
        data["revisionLocal"]
            .as_str()
            .ok_or("Missing local revision")?,
    )?;
    ...
    let remote: std::collections::HashSet<_> = remote.into_iter().map(|(hash, _)| hash).collect();
    Ok(local
        .into_iter()
        .filter(|(hash, _)| !remote.contains(hash))
        .map(|(hash, number)| format!("Revision {number} · {hash}"))
        .collect())
}

fn parse_revision_history(output: &str) -> Result<Vec<(String, String)>, String> {
    let mut revisions = Vec::new();
    let mut complete = false;
    for line in output.lines().filter(|line| !line.trim().is_empty()) {
        let event: Value = serde_json::from_str(line).map_err(|e| e.to_string())?;
        match event["tagName"].as_str() {
            Some("revisionHistoryEntry") => revisions.push((
                event["data"]["revision"]
                    .as_str()
                    .ok_or("Missing revision hash")?
                    .into(),
                event["data"]["revisionNumber"]
                    .as_u64()
                    .ok_or("Missing revision number")?
                    .to_string(),
            )),
            Some("complete") => {
...
pub fn parse_lock_status(output: &str) -> Result<bool, String> {
    ...
    count
        .map(|count| count > 0)
        .ok_or_else(|| "Missing lock status.".into())
}
...
pub fn move_entry(root: &Path, source: &Path, destination: &Path) -> Result<(), String> {
    ...
    let name = destination
        .file_name()
        .ok_or("Destination must include the new file or folder name.")?;
    let parent = destination
        .parent()
    ...
    for path in [&source, &destination] {
        let relative = path
            .strip_prefix(&root)
            .map_err(|_| "Move must stay inside the repository.")?;
...
mod tests {
    ...
    fn clone_rejects_occupied_and_relative_destinations_before_launch() {
        ...
        assert!(
            clone_repository(cli, "lores://example/repo", root.path(), None)
                .unwrap_err()
                .contains("empty folder")
        );
        assert!(
            clone_repository(cli, "lores://example/repo", Path::new("relative"), None)
                .unwrap_err()
                .contains("absolute")
        );
        assert_eq!(
    ...
    fn account_uses_returned_id_and_name_and_requires_success() {
        let info =
            "{\"tagName\":\"authUserInfo\",\"data\":{\"id\":\"uuid-123\",\"name\":\"law80\"}}";
        assert_eq!(
    ...
    fn lock_status_requires_success_and_explicit_count() {
        for (count, locked) in [(0, false), (1, true)] {
            let output = format!(
                "{{\"tagName\":\"lockFileStatusBegin\",\"data\":{{\"count\":{count}}}}}\n{{\"tagName\":\"complete\",\"data\":{{\"status\":0}}}}"
            );
    ...
    fn real_lore_local_workflow() {
        ...
        assert!(
            !parse_status(&invoke(&["status", "--scan"], true))
                .unwrap()
                .changes[0]
                .staged
        );
        ...
        assert!(
            invoke(&["file", "history", "--", "한글 sample.txt", "50"], false)
                .contains("한글 commit message")
        );
