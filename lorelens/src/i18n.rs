use std::{collections::BTreeMap, sync::{LazyLock, atomic::{AtomicUsize, Ordering}}};
...
static EN: LazyLock<BTreeMap<String, String>> = LazyLock::new(||
    serde_json::from_str(include_str!("../../i18n/en-US.json")).expect("Invalid en-US catalog"));
static KO: LazyLock<BTreeMap<String, String>> = LazyLock::new(||
    serde_json::from_str(include_str!("../../i18n/ko-KR.json")).expect("Invalid ko-KR catalog"));
static ZH: LazyLock<BTreeMap<String, String>> = LazyLock::new(||
    serde_json::from_str(include_str!("../../i18n/zh-CN.json")).expect("Invalid zh-CN catalog"));

pub fn normalize(locale: &str) -> &'static str {
    match locale { "ko-KR" => "ko-KR", "zh-CN" => "zh-CN", _ => "en-US" }
}
...
pub fn translate(locale: &str, key: &str) -> String {
    let localized = match normalize(locale) { "ko-KR" => KO.get(key), "zh-CN" => ZH.get(key), _ => None };
