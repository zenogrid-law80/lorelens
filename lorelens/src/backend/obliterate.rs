...
pub fn obliterate_args(root: &Path, path: &str) -> Result<Vec<String>, String> {
  ...
  if normalized.is_empty()
    || normalized
      .split('/')
      .any(|part| part.is_empty() || matches!(part, "." | "..") || part.contains(':') || [".git", ".lore", ".urc"].iter().any(|name| part.eq_ignore_ascii_case(name)) || part.ends_with(['.', ' ']))
  {
    return Err(invalid());
  }
  ...
  for (index, component) in components.iter().enumerate() {
    ...
    match fs::symlink_metadata(&target) {
      Ok(metadata) => {
        ...
        #[cfg(not(windows))]
        let reparse = false;
        if metadata.file_type().is_symlink() || reparse || (index + 1 == components.len() && !metadata.is_file()) {
          return Err(invalid());
        }
      }
...
mod tests {
  ...
  fn supports_missing_files_and_literal_option_like_names() {
    let root = tempfile::tempdir().unwrap();
    assert_eq!(
      obliterate_args(root.path(), "folder\\missing file.bin").unwrap(),
      ["file", "obliterate", "--path=folder/missing file.bin"]
    );
    assert_eq!(obliterate_args(root.path(), "--address=anything").unwrap()[2], "--path=--address=anything");
  }
