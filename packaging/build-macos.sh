...
for SIZE in 16 32 128 256 512; do
    sips -z "$SIZE" "$SIZE" lorelens/dist/favicon.png --out "$ICONSET/icon_${SIZE}x${SIZE}.png" >/dev/null
    DOUBLE=$((SIZE * 2))
    sips -z "$DOUBLE" "$DOUBLE" lorelens/dist/favicon.png --out "$ICONSET/icon_${SIZE}x${SIZE}@2x.png" >/dev/null
done
